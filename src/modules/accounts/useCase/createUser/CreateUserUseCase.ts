...
class CreateUserUseCase {
  ...
  async execute({
    ...
    const userDriver_license = await this.usersRepository.findByDriver_license(driver_license)
    if (userDriver_license) throw new AppError("Driver License already register!");
